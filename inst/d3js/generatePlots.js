...
function breakDown() {
  ...
  var y = d3.scaleBand()
            .rangeRound([margin.top, margin.top + bdPlotHeight])
            .padding(0.33)
...
function shapleyValues() {
  ...
  var y = d3.scaleBand()
            .rangeRound([margin.top, margin.top + svPlotHeight])
            .padding(0.33)
            .domain(bData.map(d => d.variable));
...
function featureImportance() {
  ...
  var y = d3.scaleBand()
            .rangeRound([margin.top + fiPlotHeight, margin.top])
            .padding(0.33)
            .domain(bData.map(d => d.variable));
...
function cpNumericalPlot(variableName, lData, mData, yMinMax, pData) {
  ...
  var y = d3.scaleLinear()
            .range([margin.top + cpPlotHeight, margin.top])
...
function pdNumericalPlot(variableName, lData, mData, yMinMax, yMean) {
  ...
  var y = d3.scaleLinear()
            .range([margin.top + pdPlotHeight, margin.top])
            .domain([yMinMax[0], yMinMax[1]]);
...
function adNumericalPlot(variableName, lData, mData, yMinMax, yMean) {
  ...
  var y = d3.scaleLinear()
            .range([margin.top + adPlotHeight, margin.top])
            .domain([yMinMax[0], yMinMax[1]]);
